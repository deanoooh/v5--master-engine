...
DEBUG = True  # <<< keep True to see debug panels per race
...
def extract_full_horse_name(runner):
    """
    Extract the fullest available horse name from a Sporting Life runner object.
    Checks both top-level and nested 'horse' fields.
    """
    horse_obj = runner.get("horse")
    ...
    if isinstance(horse_obj, dict):
        candidates.extend([
            horse_obj.get("name"),
            horse_obj.get("horse_name"),
            horse_obj.get("horseName"),
            horse_obj.get("display_name"),
            horse_obj.get("displayName"),
        ])
    elif isinstance(horse_obj, str):
        candidates.append(horse_obj)
...
def extract_current_odds(runner):
    """
    Extract the best available pre-race odds from a Sporting Life runner object.
    Returns a display string such as '7/2', '3.50', or 'Odds unavailable'.
    """
    # Most likely direct fields
    direct_fields = [
        "current_odds",
        "currentOdds",
        "odds",
    ...
    for field in direct_fields:
        ...
        if isinstance(value, dict):
            for nested_field in [
                ...
                if nested_value not in (None, "", "-", "SP"):
                    return str(nested_value).strip()

        elif value not in (None, "", "-", "SP"):
            return str(value).strip()

    # Common nested price structures
    for container_name in ["prices", "price", "odds_data", "bookmaker_odds"]:
        container = runner.get(container_name)
...
def extract_current_odds_from_html(horse_element):
    """
    Search the runner's surrounding HTML for a displayed price.
    Returns fractional (e.g. '7/2'), decimal (e.g. '3.50'), or 'Odds unavailable'.
    """
    parent = horse_element
    ...
    for _ in range(4):
        ...
        # Fractional odds, for example 7/2, 11/8, 5/1
        fractional = re.search(r"\b\d+\s*/\s*\d+\b", text)
        if fractional:
            return fractional.group(0).replace(" ", "")

        # Decimal odds, for example 3.50 or 6.00
        decimal = re.search(r"\b\d+\.\d{2}\b", text)
        if decimal:
    ...
    try:
        ...
        if race_id_match:
            ...
            if api_res.status_code == 200:
                ...
                # Parse runners with improved name + odds extraction
                runners = []
                raw_runners_for_debug = []  # store first few raw runner dicts for debug panel

                rides = (
                    race_info.get("rides")
                    or race_info.get("ride")
                    or race_info.get("runners")
                    or []
                )

                # Protect against an unexpected single dictionary
                if isinstance(rides, dict):
                    rides = list(rides.values())

                for idx, runner in enumerate(rides):
                    if not isinstance(runner, dict):
                        continue

                    status = str(runner.get("status", "")).upper()

                    is_non_runner = (
                        runner.get("is_non_runner", False)
                        or runner.get("isNonRunner", False)
                        or status in {"NON_RUNNER", "NON-RUNNER", "NR"}
                    )

                    if is_non_runner:
                        continue

                    # Keep a small sample of raw runner objects for debug display
                    if len(raw_runners_for_debug) < 5:
                        raw_runners_for_debug.append(runner)

                    raw_name = extract_full_horse_name(runner)

                    if not raw_name:
                        continue

                    full_name = clean_horse_name(raw_name)
                    odds = extract_current_odds(runner)

                    if full_name and not any(
                        existing["horse"].casefold() == full_name.casefold()
                        for existing in runners
                    ):
                        runners.append({
                            "horse": full_name,
                            "odds": odds
                ...
                meta = {
                    ...
                    '_debug_raw_runners': raw_runners_for_debug if DEBUG else [],
                }
                return meta, runners
        ...
        # HTML fallback for runners (with odds extraction)
        runners = []
        horse_elements = soup.find_all(
            class_=re.compile(r"HorseName|runner-name|horse-name", re.I)
        )
        ...
        for el in horse_elements:
            ...
            if raw_text and len(raw_text) > 2:
                ...
                if clean_name and not any(
                    existing["horse"].casefold() == clean_name.casefold()
                    for existing in runners
                ):
                    if not any(
                        word in clean_name.lower()
                        for word in ["club", "ltd", "racing", "stakes", "maiden"]
                    ):
                        runners.append({
                            "horse": clean_name,
                            "odds": extract_current_odds_from_html(el)
                        })
        ...
        meta = {
            ...
            'race_time': time_str,
            'active_runners': len(runners),
            '_debug_raw_runners': [],  # no API data in HTML fallback
        }
        return meta, runners
    ...
    if 'processed_races' in st.session_state and st.session_state['processed_races']:
        ...
        for race_data in st.session_state['processed_races']:
            ...
            with st.expander(f"📍 Race {r_num}: {meta.get('course')} — {meta.get('active_runners')} Runners", expanded=True):
                ...
                # =========================
                # DEBUG PANEL (API RUNNERS)
                # =========================
                if DEBUG and meta.get('_debug_raw_runners'):
                    st.markdown("---")
                    st.caption("🔧 DEBUG: Raw API runner data (first 5 runners)")

                    raw_runners = meta['_debug_raw_runners']

                    # Show a compact summary table
                    summary_rows = []
                    for i, r in enumerate(raw_runners, start=1):
                        summary_rows.append({
                            "#": i,
                            "status": r.get("status"),
                            "is_non_runner": r.get("is_non_runner") or r.get("isNonRunner"),
                            "horse_name": extract_full_horse_name(r),
                            "clean_name": clean_horse_name(extract_full_horse_name(r)),
                            "odds": extract_current_odds(r),
                        })

                    st.table(summary_rows)

                    # Allow expanding each raw runner JSON
                    with st.expander("🧪 Inspect raw runner JSON objects"):
                        for i, r in enumerate(raw_runners, start=1):
                            st.markdown(f"**Runner {i}**")
                            st.json(r)
